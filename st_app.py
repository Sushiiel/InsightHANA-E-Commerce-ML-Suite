...
def _try_read_alias(base,usecols=None):
    ...
    return None
@st.cache_data(show_spinner=False,ttl=600)
def load_from_csv(limit_rows:int=300000):
    ...
    if items is None:items=pd.DataFrame(columns=["order_id","product_id"])
    pays=_try_read_alias("olist_order_payments_dataset",usecols=["order_id","payment_installments","payment_value"])
    if pays is None:pays=pd.DataFrame(columns=["order_id","payment_installments","payment_value"])
    prods=_try_read_alias("olist_products_dataset",usecols=["product_id","product_photos_qty","product_description_lenght","product_weight_g"])
    if prods is None:prods=pd.DataFrame(columns=["product_id","product_photos_qty","product_description_lenght","product_weight_g"])
...
with c0:data_source=st.selectbox("Data Source Preference",["CSV (Olist)"],index=0)
with c1:row_limit=st.number_input("Row Limit (pull fewer rows = faster)",min_value=5000,max_value=1000000,value=150000,step=5000)
with c2:sample_for_training=st.slider("Train on % of pulled rows",min_value=10,max_value=100,value=50,step=10)
...
with tab1:
    ...
    if "order_status" in df.columns:vc=df["order_status"].value_counts();st.bar_chart(vc)
with tab2:
    if len(X)>0:
    ...
    else:X_train,y_lat_tr,y_chu_tr=X,y_late,y_churn
    with st.spinner("Training compact models..."):clf_late,clf_churn=train_models(X_train,y_lat_tr,y_chu_tr)
    st.subheader("Enter Order Details")
    c1,c2,c3=st.columns(3)
    with c1:payment_value=st.number_input("Payment Value",0.0,50000.0,200.0)
    with c2:payment_installments=st.number_input("Installments",0,60,4)
    with c3:product_photos_qty=st.number_input("Photos Qty",0,50,5)
    c4,c5,c6=st.columns(3)
    with c4:product_description_lenght=st.number_input("Description Length",0,10000,1000)
    with c5:product_weight_g=st.number_input("Weight (g)",0,100000,1000)
    with c6:purchase_dayofweek=st.selectbox("Day of Week (0=Mon)",list(range(7)),index=0)
...
with tab3:
    ...
    try:
        ...
        else:fi=pd.Series({c:0.0 for c in FEATURE_COLS})
    except Exception:
        def _corr(a,b):
